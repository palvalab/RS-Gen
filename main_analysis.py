...
subdirectory  = data_dir + 'N83_rest_' + datatype + '_' + parcs1[parcind] + '\\'
ylabel        = datatype
...
if dind==2:
    data1         = np.zeros([N_freq,83,N_parc,N_parc],'single')
    data_flat     = np.zeros([N_freq,83,N_parc*N_parc],'single')
    if parc == 'parc2009':
        DEM_file  = data_dir + 'DEM_matrix ' + parc + '.csv' 
        DEM_used  = np.reshape(np.genfromtxt(DEM_file,delimiter=';'),148*148)
    else:
        DEM_used  = np.ones(N_parc*N_parc)
    
else:
    data1         = np.zeros([N_freq,83,N_parc],'single')
    data_flat     = np.zeros([N_freq,83,N_parc],'single')
...
for f,ff in enumerate(frequencies):
     ...
     if dind==2:
         data1[f]      = np.reshape(data_flat[f],(83,N_parc,N_parc))
     else:
         data1[f]      = np.reshape(data_flat[f],(83,N_parc))
         
     print(ff)

       
# delete outlier subject    
data_flat  =  np.delete(data_flat,60,1)
data1      =  np.delete(data1,60,1)   
...
networks_file         = data_dir + 'networks ' + parc + '.csv'
...
parcels = np.genfromtxt(data_dir + 'parcels.csv',dtype='str',delimiter=';')[:,0]

parcel_conv = np.genfromtxt(data_dir + 'parc2009_py_vs_LV.csv',dtype='str',delimiter=';')
